...
aicp <- AIC(pM0, pMl0, pM0b, pMlb)
aicp$dAIC <- aicp$AIC-min(aicp$AIC)

cbind(aict, t(sapply(list(tM0, tMl0, tM0b, tMlb),
    function(z) z$summary[c("sigma","lambda"), 1])))

cbind(aicp, t(sapply(list(pM0, pMl0, pM0b, pMlb),
    function(z) z$summary[c("sigma","lambda"), 1])))
...
library(phytools)
tre <- compute.brlen(vcv2phylo(cor_matrix))
stopifnot(all(tre$tip.label == rownames(lhreg_data)))
NAMES <- abbreviate(lhreg_data$common_name, 12, named=FALSE)
tre$tip.label <- NAMES
...
#library(viridis) # use viridis, plasma, magma etc
#col_fun <- colorRampPalette(viridis(100)[20:100])
#col_fun <- colorRampPalette(grey(seq(0.8, 0, by=-0.1))) # grey scale
col_fun <- colorRampPalette(Col) # red-yl-blue
op <- par(mfrow=c(1,2))
contMap2(tre, structure(exp(lhreg_data$logphi), .Names=NAMES),
    fsize=0.5, outline=FALSE, col_fun=col_fun, main="SR")
contMap2(tre, structure(exp(lhreg_data$logtau), .Names=NAMES),
    fsize=0.5, outline=FALSE, col_fun=col_fun, direction="leftwards", main="DD")
par(op)
```

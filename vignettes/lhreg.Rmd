...
tree <- vcv2phylo(cor_matrix)
Mp <- lhreg(Y=qlogis(po), X=X0, SE=0, V=vc, lambda=NA, hessian=TRUE, method="DE")
Mq <- lhreg(Y=qlogis(qo), X=X0, SE=0, V=vc, lambda=NA, hessian=TRUE, method="DE")
Mpq <- lhreg(Y=qlogis(pq), X=X0, SE=0, V=vc, lambda=NA, hessian=TRUE, method="DE")
...
B <- 200
pdp <- lapply(c(0,0.5,1), f, mu=po, T=tree)
pdq <- lapply(c(0,0.5,1), f, mu=qo, T=tree)
pdpq <- lapply(c(0,0.5,1), f, mu=pq, T=tree)
...
rx <- lapply(c(0,0.5,1), f, mu=rep(0.5, NS), T=tree)
par(mfrow=c(1,1))
plot(density(rx[[1]]), xlim=c(0,max(unlist(rx))), main="lambda=0")
lines(density(rx[[2]]), col=2)
lines(density(rx[[3]]), col=4)

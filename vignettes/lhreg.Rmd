...
due to issues with boundary condition when traits used for DD model.
...
r_fun1 <- function() {
    #r0 <- t(replicate(1, rbinom(NS, 1, pqo[sample.int(NS)])))
    r0 <- t(replicate(1, ifelse(pqo[sample.int(NS)]>0.5,1,0)))
    colnames(r0) <- rownames(lhreg_data)
    pd(r0, tree)$PD
}
r_fun <- function()
    do.call(rbind, lapply(1:B, function(x) r_fun1()))

tree <- compute.brlen(vcv2phylo(cor_matrix))
B <- 100
tt <- 10
rr <- 1

po <- sr_fun(tt, exp(lhreg_data$logphi))
qo <- dd_fun(rr, exp(lhreg_data$logtau))
pq <- po *qo
NS <- length(pq)
summary(pq)
plot(data.frame(p=po, q=qo, pq=pq))

#pqo <- po
#pqo <- qo
pqo <- pq

#ro <- t(replicate(B, rbinom(NS, 1, pqo)))
ro <- t(replicate(B, ifelse(pqo>0.5,1,0)))
colnames(ro) <- rownames(lhreg_data)
pdo <- pd(ro, tree)$PD
pd0 <- r_fun()
...
simp <- exp(simulate(pMlx, B))
simt <- exp(simulate(tMlx, B))

pq <- sr_fun(tt, simp) * dd_fun(rr, simt)
pd <- pd(ifelse(pq > 0.5, 1, 0), tree)
summary(pd)

simp2 <- exp(simulate(pMlx, B, lambda=0))
simt2 <- exp(simulate(tMlx, B, lambda=0))

pq2 <- sr_fun(tt, simp2) * dd_fun(rr, simt2)
pq20 <- ifelse(pq2 > 0.5, 1, 0)
pd2 <- pd(pq20, tree)
summary(pd2)

f <- function(lam=0) {
    simp2 <- exp(simulate(pMpx, B, lambda=lam))
    simt2 <- exp(simulate(tMpx, B, lambda=lam))
    pq2 <- sr_fun(tt, simp2) * dd_fun(rr, simt2)
    pd(ifelse(pq2 > 0.5, 1, 0), tree)
}

ppd <- lapply(c(0,0.5,1), f)

plot(ppd[[1]], col=2, xlim=c(0,100), ylim=c(0,100))
points(ppd[[2]], col=3)
points(ppd[[3]], col=4)

par(mfrow=c(3,2))
hist(ppd[[1]][,1], xlim=c(0,40), col=1)
hist(ppd[[1]][,2], xlim=c(0,40), col=1)
hist(ppd[[2]][,1], xlim=c(0,40), col=2)
hist(ppd[[2]][,2], xlim=c(0,40), col=2)
hist(ppd[[3]][,1], xlim=c(0,40), col=3)
hist(ppd[[3]][,2], xlim=c(0,40), col=3)

mup <- rep(mean(lhreg_data$logphi), nrow(lhreg_data))
mut <- rep(mean(lhreg_data$logtau), nrow(lhreg_data))
ff <- function(lam) {
    VV <- cor_matrix
    VV[lower.tri(VV)] <- lam * VV[lower.tri(VV)]
    VV[upper.tri(VV)] <- lam * VV[upper.tri(VV)]
    p <- sr_fun(tt, exp(mvtnorm::rmvnorm(B, mup, VV)))
    q <- dd_fun(rr, exp(mvtnorm::rmvnorm(B, mut, VV)))
    pd(ifelse(p*q > 0.5, 1, 0), tree)
}
summary(ff(0))
summary(ff(0.5))
summary(ff(1))

tree1 <- vcv2phylo(cor_matrix)
tree1 <- compute.brlen(tree1)
vc1 <- vcv(tree1, corr=TRUE)
Mp <- lhreg(Y=qlogis(po), X=X0, SE=0, V=vc1, lambda=NA, hessian=TRUE, method="DE")
Mq <- lhreg(Y=qlogis(qo), X=X0, SE=0, V=vc1, lambda=NA, hessian=TRUE, method="DE")
Mpq <- lhreg(Y=qlogis(pq), X=X0, SE=0, V=vc1, lambda=NA, hessian=TRUE, method="DE")

summary(Mp)
summary(Mq)
summary(Mpq)

tree2 <- rtree(141, tip.label=rownames(lhreg_data))
tree2 <- compute.brlen(tree2)
vc2 <- vcv(tree2, corr=TRUE)
rMp <- lhreg(Y=qlogis(po), X=X0, SE=0, V=vc2, lambda=NA, hessian=TRUE, method="DE")
rMq <- lhreg(Y=qlogis(qo), X=X0, SE=0, V=vc2, lambda=NA, hessian=TRUE, method="DE")
rMpq <- lhreg(Y=qlogis(pq), X=X0, SE=0, V=vc2, lambda=NA, hessian=TRUE, method="DE")
...
f <- function(lam=0, M, T) {
    VV <- M$V
    VV[lower.tri(VV)] <- lam * VV[lower.tri(VV)]
    VV[upper.tri(VV)] <- lam * VV[upper.tri(VV)]
    sim <- plogis(mvtnorm::rmvnorm(B, M$Y, VV))
    #sim <- plogis(simulate(M, B, lambda=lam))
    colnames(sim) <- rownames(lhreg_data)
    pd(ifelse(sim > 0.5, 1, 0), T)$PD
}

f <- function(lam=0, mu, T) {
    VV <- vcv(T, corr=TRUE)
    VV[lower.tri(VV)] <- lam * VV[lower.tri(VV)]
    VV[upper.tri(VV)] <- lam * VV[upper.tri(VV)]
    sim <- plogis(mvtnorm::rmvnorm(B, mu, VV))
    colnames(sim) <- rownames(lhreg_data)
    pd(ifelse(sim > 0.5, 1, 0), T)$PD
    #pd_quick(ifelse(sim > 0.5, 1, 0), T)
}

B <- 200
pdp <- lapply(c(0,0.5,1), f, mu=po, T=tree1)
pdq <- lapply(c(0,0.5,1), f, mu=qo, T=tree1)
pdpq <- lapply(c(0,0.5,1), f, mu=pq, T=tree1)

rpdp <- lapply(c(0,0.5,1), f, mu=po, T=tree2)
rpdq <- lapply(c(0,0.5,1), f, mu=qo, T=tree2)
rpdpq <- lapply(c(0,0.5,1), f, mu=pq, T=tree2)

MAX <- max(unlist(pdp), unlist(pdq), unlist(pdpq),
    unlist(rpdp), unlist(rpdq), unlist(rpdpq))

par(mfrow=c(2,3))

plot(density(pdp[[1]]), xlim=c(0,MAX), main="lambda=0")
lines(density(pdp[[2]]), col=2)
lines(density(pdp[[3]]), col=4)
plot(density(pdq[[1]]), xlim=c(0,MAX), main="lambda=0.5")
lines(density(pdq[[2]]), col=2)
lines(density(pdq[[3]]), col=4)
plot(density(pdpq[[1]]), xlim=c(0,MAX), main="lambda=1")
lines(density(pdpq[[2]]), col=2)
lines(density(pdpq[[3]]), col=4)

plot(density(rpdp[[1]]), xlim=c(0,MAX), main="lambda=0")
lines(density(rpdp[[2]]), col=2)
lines(density(rpdp[[3]]), col=4)
plot(density(rpdq[[1]]), xlim=c(0,MAX), main="lambda=0.5")
lines(density(rpdq[[2]]), col=2)
lines(density(rpdq[[3]]), col=4)
plot(density(rpdpq[[1]]), xlim=c(0,MAX), main="lambda=1")
lines(density(rpdpq[[2]]), col=2)
lines(density(rpdpq[[3]]), col=4)


B <- 100
rx <- sapply(seq(0,1,0.1), f, mu=rep(0.5, NS), T=tree1)
boxplot(rx)

par(mfrow=c(1,1))
plot(density(rx[,1]), xlim=c(0,max(unlist(rx))))
lines(density(rx[,6]), col=2)
lines(density(rx[,11]), col=4)
